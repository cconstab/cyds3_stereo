...
// Register I/O via Arduino Wire — the bus is shared with the FT6336U touch
// controller, so all codec I2C access must stay on core 1 (setup / UI / web).
...
// CYD-S3 Stereo: configure the codec as an I2S slave DAC fed by the mirrored
// stereo bus. ESP32-audioI2S generates MCLK = 128 x fs; the ratio-128 divider
// rows in coeff_div[] are identical for every sample rate, so programming the
// 44.1kHz entry covers 32/44.1/48kHz streams alike.
static es8311_handle_t s_codec = nullptr;

esp_err_t es8311_codec_init(void)
{
    s_codec = es8311_create(I2C_NUM_0, ES8311_ADDRRES_0);
    ESP_RETURN_ON_FALSE(s_codec, ESP_FAIL, TAG, "es8311 create failed");
    const es8311_clock_config_t es_clk = {
        .mclk_inverted = false,
        .sclk_inverted = false,
        .mclk_from_mclk_pin = true,
        .mclk_frequency = 44100 * 128,
        .sample_frequency = 44100
    };

    ESP_RETURN_ON_ERROR(es8311_init(s_codec, &es_clk, ES8311_RESOLUTION_16, ES8311_RESOLUTION_16), TAG, "es8311 init failed");
    ESP_RETURN_ON_ERROR(es8311_sample_frequency_config(s_codec, 44100 * 128, 44100), TAG, "set es8311 sample frequency failed");
    ESP_RETURN_ON_ERROR(es8311_voice_volume_set(s_codec, 90, NULL), TAG, "set es8311 volume failed");
    return ESP_OK;
}

esp_err_t es8311_output_enable(bool on)
{
    if (!s_codec) return ESP_ERR_INVALID_STATE;
    return es8311_voice_mute(s_codec, !on);
}
